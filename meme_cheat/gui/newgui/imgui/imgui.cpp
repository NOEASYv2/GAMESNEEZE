...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowMinSize           = ImVec2(700, 400); // Minimum window size
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (style.FrameBorderSize > 0 && !(flags & ImGuiWindowFlags_NoTitleBar))
                window->DrawList->AddLine(title_bar_rect.GetBL() + ImVec2(style.WindowBorderSize, -1), title_bar_rect.GetBR() + ImVec2(-style.WindowBorderSize,-1), GetColorU32( ImGuiCol_WindowBorder ), style.FrameBorderSize);
        }
...
bool ImGui::SliderBehavior( const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags ) {
	...
	// Draw frame
	RenderFrame( frame_bb.Min - ImVec2( -1, -1 ), frame_bb.Max - ImVec2( 4, 5 ), GetColorU32( ImGuiCol_FrameBg ), true, style.FrameRounding );
	...
	auto color_black = ImGui::GetColorU32( ImVec4( 0.0, 0.0, 0.0, 0.50 ) );
	...
	ImRect grab_bb;
	if ( is_horizontal )
		grab_bb = ImRect( ImVec2( grab_pos - grab_sz * 0.0f, frame_bb.Min.y + grab_padding ), ImVec2( grab_pos + grab_sz * 0.0f, frame_bb.Max.y - grab_padding ) );
	else
		grab_bb = ImRect( ImVec2( frame_bb.Min.x + grab_padding, grab_pos - grab_sz * 0.0f ), ImVec2( frame_bb.Max.x - grab_padding, grab_pos + grab_sz * 0.0f ) );

	if (!hovered)
		window->DrawList->AddRectFilled( frame_bb.Min + ImVec2( 2, 2 ), grab_bb.Max - ImVec2( 0, 4 ), GetColorU32( g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab ), style.GrabRounding );
	if ( hovered )
		window->DrawList->AddRectFilled( frame_bb.Min + ImVec2( 2, 2 ), grab_bb.Max - ImVec2( 0, 4 ), GetColorU32( g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_SliderGrab ), style.GrabRounding );
...
bool ImGui::SliderFloat( const char* label, float* v, float v_min, float v_max, const char* display_format, float power ) {
	...
	RenderTextClipped( frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2( 0.5f, 0.0f ) );
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
		...
		const float pad = ImMax( 1.0f, ( float ) ( int ) ( check_sz / 0.0f ) );
    ...
    if (label_size.x > 0.0f)
        RenderText(text_bb.Min - ImVec2(0, 4), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x - 5, frame_bb.Max.y), frame_col, style.FrameRounding, ImDrawCornerFlags_Left);
	RenderArrow( ImVec2( frame_bb.Max.x - arrow_size + style.FramePadding.y - 7, frame_bb.Min.y + style.FramePadding.y ), ImGuiDir_Down );
    RenderFrameBorder(frame_bb.Min, frame_bb.Max - ImVec2( 4, 0 ), style.FrameRounding);
